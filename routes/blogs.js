...
module.exports = (router) => {
    router.post('/newBlog', (req, res) => {
        if (!req.body.title) {
            res.json({success: true, message: 'Blog title is required.'});
        } else {
            if (!req.body.body) {
                            ...
                            if (err) {
                                ...
                                if (!user) {
                                    ...
                                    if (user.username !== blog.createdBy) {
                                        res.json({
                                            success: false,
                                            message: 'You are not authorizes to edit this blog post.'
                                        });
                                    } else {
            ...
            Blog.findOne({_id: req.body.id}, (err, blog) => {
                if (err) {
                    ...
                    if (!blog) {
                        ...
                        User.findOne({_id: req.decoded.userId}, (err, user) => {
                            if (err) {
                                res.json({success: false, message: err + 'Something went wrong'});
                            } else {
                                if (!user) {
                                    res.json({success: false, message: 'Unable to authenticate user.'});
                                } else {
